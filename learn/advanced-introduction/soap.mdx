...
SOAP (Subsidized Open-market Acquisition Protocol) turns merge-mining rewards into continuous QUAI buybacks. Purchased QUAI is burned (100%).
...
SOAP transforms merge-mining into a protocol subsidy mechanism. Instead of miners receiving the parent chain's block reward directly, SOAP routes 100% of the parent chain's coinbase output to a protocol-controlled address. That address then converts the parent chain tokens to QUAI at market and burns the purchased QUAI.

Think of it this way: when you mine QUAI with SOAP enabled, you earn QUAI block rewards as you normally would. SOAP doesn't change that. What SOAP adds is the ability to also contribute hashrate from other chains (BCH, LTC, DOGE, RVN) through workshares/blocks to provide a protocol subsidy.

The economic proposition for participating in SOAP is straightforward: you're mining QUAI and getting paid in QUAI, but your parent chain block subsidy goes to the protocol.

### Example of SOAP flows

For miners focused purely on QUAI, nothing changes. They mine SHA-256d blocks and earn QUAI. For miners with existing BCH/LTC/DOGE hardware, SOAP offers a way to now earn QUAI. You simply point your SHA256d or Scrypt rigs at SOAP-enabled pools to mine QUAI while the merge-mined BCH/LTC/DOGE is used for buybacks.
...
In the worst case, this creates net-zero daily flow: miners sell their QUAI, the protocol buys QUAI with parent chain subsidies, the forces cancel. But in practice, some miners are "non-mercenary" (they hold rather than immediately dumping) which means most days produce net positive buy pressure for QUAI.

More subsidy chain funded buying leads to price improvement, which leads to more hashrate, which leads to stronger QUAI security, which makes subsidy chains find SOAP more attractive, which leads to more subsidy flows, which leads to more buy and burn.
...
- Metadata identifying which parent chain this is from (BCH, LTC, DOGE, RVN, etc.)
...
Miners vary `extranonce2` as usual, pools provide `extranonce1`, and the SOAP push is fixed by the pool's job template. This means standard Stratum v1 mining continues to work without firmware changes. The pool specifies the QUAI payout address that will receive workshare rewards.
...
Here's where SOAP gets interesting from a protocol design perspective. Quai blocks are authored exclusively by SHA-256d miners. No other algorithm can create a Quai block. But other algorithms (SHA256d from BCH, Scrypt from LTC/DOGE, and KAWPOW from RVN) can submit workshares that get included in SHA-256d blocks and earn QUAI rewards.

Quai developed workshares initially to prevent selfish mining and provide hashrate samples for faster finality convergence, but the underlying mechanism naturally extends to support multiple proof-of-work algorithms beyond just SHA-256d.
...
- Do **not** meet Quai's block difficulty target (not SHA-256d blocks)
...
Each workshare proves:
- "I performed computation on chain X (BCH/LTC/DOGE/RVN), I referenced Quai WorkObject Y, and my parent chain block paid QADDR."

The workshare doesn't create a new Quai block, but it does create a QUAI payment to the address specified in the WorkObject's `primaryCoinbase` field.
...
- **Scalability of subsidy conversion:** If Quai's SHA-256d blocks are already being produced at target rate, workshares provide a mechanism to continue scaling parent chain participation without destabilizing Quai's block timing or difficulty adjustment.
...
A KAWPOW block including SHA256d and Scrypt workshares has measurably more weight than an equivalent KAWPOW block without them. This additional weight makes the block harder to reorg and contributes to faster economic finality.
...
- Each block accumulates more entropy than SHA-256d alone would provide
- Reorg attacks become proportionally more expensive (attacker must overcome both block difficulty and workshare entropy)
- Multi-algorithm security compounds (attacker needs hashrate across multiple algorithms or must produce enough SHA-256d work to overcome the workshare entropy deficit)

Example:
- A block with 1000 SHA-256d entropy plus 200 workshare entropy (150 Script, 50 KAWPOW) totals 1200 units.
- An attacker must produce 20% more work to reorg compared to a SHA-256d-only chain.
...
- They receive QUAI rewards from Quai's emission schedule in proportion to their workshare submissions.
...
**Quai SHA-256d block producers**

- They receive standard QUAI block rewards for producing blocks.
- They're securing Quai directly using SHA-256d and benefit from SOAP buy pressure on QUAI since the protocol continuously converts BCH/LTC/DOGE subsidies into QUAI purchases.
...
Templates are signed by a threshold of committee members (for example, 2-of-3 multisig), and Quai consensus requires that any included AuxPoW must correspond to a valid, quorum-signed template.
...
A final note on the security model: SOAP is designed to fail gracefully. If parent chain participation drops to zero (no BCH/LTC/DOGE/RVN miners submit workshares), Quai keeps producing SHA-256d blocks normally. There's no liveness coupling. Workshares are purely additive.

This is in contrast to some merge-mining setups where the child chain depends on parent chain blocks to maintain consensus. If you required every Quai block to include at least one parent chain AuxPoW, Quai's liveness would be coupled to parent chain production and become fragile.
...
SOAP transforms traditional merge-mining into a protocol subsidy mechanism by routing parent chain subsidies to protocol-controlled addresses used to buyback QUAI. The purchased QUAI is then burned. This creates continuous buy pressure while allowing external miners to earn QUAI through workshares.

Technically, SOAP uses AuxPoW structures to prove parent chain participation, with commitments placed in coinbase scriptSigs to maintain compatibility with standard mining infrastructure. Quai blocks are SHA-256d-only (transitioning from ProgPoW and KAWPOW), but workshares from other algorithms (SHA256d, Scrypt, and KAWPOW) can be included for QUAI rewards, diversifying the security base across different hardware classes.
